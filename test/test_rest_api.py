...
def _get_converted(timestamp_: datetime.datetime, uuid_: uuid.UUID):
    jpg = model.JPG(code=uuid_, src_path="random.jpg")
    pdf = model.PDF(code=uuid_, destination_path="random.pdf")
    return {"converted_from": jpg, "converted_to": pdf, "converted_at": timestamp_}
